---
title: "CKME136 - Final Code"
author: "Allan Mariano"
date: "August 1, 2016"
output: pdf_document
---

##Initialization - Install/Load packages
```{r, echo=FALSE}
...
```{r}
pets <- read.csv("https://raw.githubusercontent.com/allanreym/ShelterAnimal/master/data_combined.tsv", 
                 header = T, sep = "\t")
...
pets$AgeuponOutcome_Years <- 
  ifelse(grepl(pattern = "week", tolower(TimeUnit)), TimeNum / 52,
...
```{r}
pets$SexuponOutcome_Sex <- as.factor(
  ifelse(grepl(pattern = " male", tolower(pets$SexuponOutcome)), "male",
...
```{r}
pets$SexuponOutcome_Neutered <- as.factor(
  ifelse(grepl(pattern = "intact", tolower(pets$SexuponOutcome)), "intact",
...
##Transform - Add new data frame columns of unique Color values
for (color in Color_lvl)
  {pets[make.names(paste(color, "Color"))] <- as.numeric(grepl(color, pets$Color))}
...
pets_colrm <- c("AnimalID", "Name", "DateTime", "OutcomeSubtype", "SexuponOutcome", 
                "AgeuponOutcome", "Breed1", "Breed2", "BreedAKC1", "BreedAKC2", 
                "BreedPetfinder1", "BreedPetfinder2", "Size1", "Size2", "Energy1", 
                "Energy2", "Friendliness.to.Other.Pets", "Breed")
...
for (outcome in OutcomeType_lvl)
  {pets_numeric[make.names(outcome)] <- as.numeric(grepl(outcome, pets_numeric$OutcomeType))}
```
...
for (neutered in Neutered_lvl)
{pets_numeric[make.names(neutered)] <- 
    as.numeric(grepl(neutered, pets_numeric$SexuponOutcome_Neutered))}
```
##Visualization 1 - Create SexuponOutcome_Neutered indicators
```{r}
Sex_lvl <- make.names(sort(unique(as.character(pets_numeric$SexuponOutcome_Sex))))
for (sex in Sex_lvl)
{pets_numeric[make.names(sex)] <- 
    as.numeric(grepl(paste("\\b", sex, sep = ""), pets_numeric$SexuponOutcome_Sex))}
```

##Visualization 1 - Reduce pets_numeric features to those with most information gain
```{r}
pets_numeric <- 
  pets_numeric[,c(OutcomeType_lvl, Neutered_lvl, Sex_lvl,
...
```{r}
pets_neut <- 
  pets_kaggle_train[which((pets_kaggle_train$OutcomeType == "Adoption" |
                           pets_kaggle_train$OutcomeType == "Return_to_owner") &
                           pets_kaggle_train$SexuponOutcome_Neutered != "unknown neutered"), 
                           c("OutcomeType", "SexuponOutcome_Neutered")]
...
ggplot(pets_neut, aes(x = OutcomeType)) + 
  geom_bar(aes(fill = SexuponOutcome_Neutered), position = 'fill') +
...
```{r}
pets_age <- 
  pets_kaggle_train[which(pets_kaggle_train$AgeuponOutcome_Years > 0 &
                            (pets_kaggle_train$OutcomeType == "Adoption" |
                            pets_kaggle_train$OutcomeType == "Euthanasia")), 
                    c("OutcomeType", "AgeuponOutcome_Years")]
